...
| `Enter` | Activate the selection: expand/collapse a directory, or open a file in **zoom mode** (content full-screen). An **image/video** file auto-displays inline as soon as it's highlighted (no keypress) on a graphics-capable terminal; `Enter` then just zooms it larger (see [renderers](renderers.md#inline-image--video-preview)) |
...
| `g` | **Follow a wikilink**: when viewing a markdown note inside an Obsidian vault, open the link navigator — a list of the note's `[[wikilinks]]` / `![[embeds]]` / relative markdown links; `↑` / `↓` or `j` / `k` move, `Enter` follows the selected link to that note in-viewer, `Esc` / `q` closes. Targets resolve vault-wide (shortest-unique-path); an unresolved link is marked and never followed; a link with a `#heading` / `#^block` anchor scrolls the opened note to it |
| `[` / `]` | **Back / forward** through the notes visited via link navigation (a browser-style history); a notice when there is nothing to go back / forward to |
| `o` | **Heading outline**: when viewing a markdown note, open a jump-through list of its headings, indented by level; `↑` / `↓` or `j` / `k` move, `Enter` scrolls the content pane to the selected heading, `Esc` / `q` closes. A notice when the file is not markdown or has no headings. Since rendered markdown has no source map, jumping switches the note to the line-numbered source view and scrolls there (like go-to-line) |
| `F` (Shift+`f`) | **Quick-switch note**: when the current selection is inside an Obsidian vault, open a fuzzy finder over **every note in the vault** (Obsidian's ⌘O); type to filter by note name or frontmatter **alias**, `↑` / `↓` move, `Enter` opens the chosen note in-viewer, `Esc` cancels. A notice when not in a vault. Distinct from `f` (go-to-**file**, the repo's files): `F` finds vault **notes** by name/alias, vault-wide |
| `S` (Shift+`s`) | **Search vault contents**: when inside an Obsidian vault, search **every note's text** for a query (uses ripgrep when installed, a built-in fallback otherwise). Type your query and press `Enter` to run the search; `↑` / `↓` move through the hits (each shows `note:line` and a preview), `Enter` opens the highlighted hit in-viewer scrolled to the matched line, `Esc` cancels. Editing the query and pressing `Enter` re-runs the search. Literal, smartcase matching (like `/`). Distinct from `/` (search *within* the open file) |
| `G` (Shift+`g`) | **Backlinks panel** ("what links here"): when viewing a markdown note inside an Obsidian vault, open a list of **every other note that links to it** (a `[[wikilink]]` / `![[embed]]` / markdown link resolving to this note); `↑` / `↓` or `j` / `k` move, `Enter` opens the selected note in-viewer (keeping the `[` / `]` back/forward history), `Esc` / `q` closes. The inbound mirror of `g` (`g` = links **out** of this note, `G` = links **in**); a notice when the file is not a markdown note in a vault or nothing links to it. Backed by the same cached vault index the quick-switcher uses (rebuilt on worktree switch / `r` refresh) |
...
| `p` | **Properties panel**: show / hide the frontmatter Properties table at the top of the rendered markdown view (like Obsidian). Shown by default; a no-op outside a rendered markdown note |
