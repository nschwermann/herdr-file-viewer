...
| `media` | The read-only, testable *decisions* for inline image/video preview: classify a path, parse image dimensions from the header (bounded read), detect the terminal's (env) graphics protocol + video poster tool, build the ffmpeg poster argv, and format the metadata placeholder. The pixels are painted by `app::MediaPane` (owns the `ratatui-image` picker + the loaded image protocol, draws it into the content pane below the metadata each frame); this module never emits escape bytes. |
| `presenter` | Draw the two-column (or zoomed / narrow) layout with ratatui, including persistent annotation markers and background-only styling; source-line backgrounds are applied beneath active line-select, ambient-selection, and search overlays, with a bounded one-cell cue for blank annotated lines. Draw the content pane's bottom **status bar** (view-type + links chips, annotation count, `? help`) from a single shared layout so the drawn chips and their fed-back clickable rects (`status_view_rect` / `status_links_rect`) can't drift. Scroll the tree/content and report viewport + pane geometry back for hit-testing. |
...
| `vault_index` | A lazily-built, cached snapshot of the containing Obsidian vault: every `.md` note keyed by vault-relative path + name + frontmatter aliases, plus each note's resolved outgoing links — the shared foundation the quick-switcher and backlinks panel read (backlinks is a reverse lookup over the outgoing links). Read-only, bounded, full-scan; rebuilt on re-root / refresh. |
...
| `vsearch` | Vault content search: grep every note's body for a query — ripgrep (`rg`) when present, a pure in-Rust fallback otherwise — returning `note:line + preview` hits (literal, smartcase, bounded). The `rg` call sits behind the injected `ContentSearcher` seam so the controller/tests never spawn a subprocess. Read-only. |
...
| `mdstyle` | The rendered-markdown "Obsidian glowup" post-render pass: over `glow`'s ingested `Text`, restyle wikilink / embed / markdown-link markup in a distinct underlined link colour and turn callouts into titled boxes (per-type accent title + left bar over a faint background tint). Reuses `wikilink::parse_links` on the pre-glow source to skip code, and mirrors `highlight`'s span re-segmentation. Pure; rendered-markdown view only (never source / diffs). |
| `tagindex` | A cached, read-only snapshot of the containing Obsidian vault's tags (`tag → note paths`) backing the clickable-tag tree filter: a bounded, dot-folder-skipping walk parses each note's frontmatter `tags:`/`tag:` and its inline `#tag` hashtags (fence-aware; a heading `# ` and a purely numeric `#123` are not tags), normalizes them case-insensitively, and indexes each nested tag under its full path and every ancestor prefix (Obsidian's `tag:` semantics). Pure parsing surface; only `build` touches the filesystem. |
...
| `controller` | Orchestrate intents → state changes; hold ephemeral session state, including the root-bound `AnnotationStore`; dispatch renders to the worker; map mouse events against fed-back geometry; and rebuild root-bound services on a worktree switch. Its owned annotation projection root-joins file targets, follows the applied `content_path` rather than the live cursor, and exposes merged line ranges only when the applied render carries a source map. Feature submodules are `mod`, `mouse`, `help`, `finder`, `picker`, `infile`, `lineselect`, `annotation`, `git_apply`, `linknav`, `outline`, `tagfilter` (the clickable-tag tree filter: build/cache the vault `tagindex`, apply the match set to the tree, and clear it on `Esc`), `switcher` (the vault quick-switcher, plus the shared vault-detection / cached-index helpers), `globalsearch` (the vault content search), and `backlinks` (the "what links here" panel: a reverse lookup over the cached vault index's outgoing links — the inbound mirror of `linknav`). One `Modal` enum type-enforces exclusive input ownership. Only a successful root change clears annotations; failure and same-root paths preserve them. Quitting or switching worktree with a non-empty store raises the `Modal::DiscardConfirm` layer (for a quit, outside search/unzoom) rather than silently discarding it; its `y` proceeds only on a successful clipboard write, and a switch re-validates its held target before committing. |
