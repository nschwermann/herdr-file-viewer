...
### Added
- **Interactive status bar** on the content pane's bottom border: it shows the current **view type** (`Markdown` / `Source` / `Diff` / `Full Diff`) and, for a markdown note in an Obsidian vault, a **links counter** (`3 links`), alongside the persistent `? help` hint and the annotation count. **Clicking the view-type chip cycles the view** (same as `v`); **clicking the links counter opens the note's link list** (same as `g`). Keyboard behaviour is unchanged, and the pane title still shows just the file name. Chips shed least-important-first on a narrow pane. → [keys](docs/keys.md#mouse)
- **Click a `#tag` to filter the tree**: in a markdown-in-vault note, clicking a tag chip in the frontmatter Properties panel — or an inline `#tag` in the body — restricts the file tree to every vault note carrying that tag (Obsidian's `tag:` search; a parent tag also matches its nested children, case-insensitively). The tree title shows `▽ #tag`; `Esc` clears it and restores the full tree. Only notes under the tree root are shown. → [usage](docs/usage.md#filtering-the-tree-by-tag-obsidian-vaults)
- **Click a link to follow it**: in a markdown-in-vault note, clicking a `[[wikilink]]` / `![[embed]]` / `[text](note)` opens that note in-viewer (same resolution + back/forward history as the `g` navigator). A click on plain text just focuses the pane. → [keys](docs/keys.md#mouse)
- **Inline image embeds in rendered markdown**: an Obsidian `![[image.png|width]]` (resolved same-folder, then vault-wide) or a standard `![](path.png)` now renders the **image inline** at the embed location in the `v` rendered view (scroll-aware), instead of showing the literal markup. Respects the `|width` hint, capped to a preview height (select the file itself for a full view); needs the same graphics terminal as the standalone preview, and is a no-op without one. → [usage](docs/usage.md#obsidian-markdown-rendering)
...
- **Heading outline** for a markdown note: `o` opens a jump-through list of the note's headings (indented by level); `Enter` scrolls the content pane to the selected heading. Since rendered markdown has no source map, jumping switches the note to the line-numbered source view and scrolls there (like go-to-line). → [usage](docs/usage.md#heading-outline)
- **Vault quick-switcher**: `F` opens a fuzzy finder over every note in the containing Obsidian vault (Obsidian's ⌘O), matched by note name or frontmatter alias; `Enter` opens the chosen note in-viewer and records it in the `[`/`]` history. Distinct from `f` (go-to-file, the repo's files). Backed by a cached, read-only index of the vault, rebuilt on worktree switch / `r` refresh. → [usage](docs/usage.md#quick-switch-to-any-note-obsidian-vaults)
- **Vault content search**: `S` searches every note's *text* across the vault (ripgrep when installed, a built-in fallback otherwise); type a query and `Enter` to run it, `↑`/`↓` through the `note:line` hits, `Enter` opens the chosen hit in-viewer at the matched line. Literal, smartcase (like `/`); distinct from `/` (search within the open file). → [usage](docs/usage.md#searching-the-whole-vault-obsidian-vaults)
- **Backlinks panel** ("what links here"): `G` opens a list of every other note that links *to* the current markdown-in-vault note (a `[[wikilink]]` / `![[embed]]` / markdown link resolving to it); `Enter` opens the selected note in-viewer, keeping the `[`/`]` history. The inbound mirror of `g` (`g` = links out, `G` = links in), computed as a reverse lookup over the same cached vault index (rebuilt on worktree switch / `r` refresh). → [usage](docs/usage.md#backlinks--what-links-here-obsidian-vaults)
- **Frontmatter Properties panel**: a note's YAML frontmatter renders as a clean key/value Properties table (tags/aliases as chips, URLs as links) at the top of the rendered markdown view, instead of being hidden. `p` shows / hides it, like Obsidian. → [usage](docs/usage.md#obsidian-markdown-rendering)
- **Obsidian callouts** in the rendered markdown view: `> [!note]` / `[!warning]` / `[!tip]` … blocks render as titled boxes with a **per-type accent colour** (note=blue, tip=cyan, warning=yellow, danger=red, …) — an accent title and left bar over a faint filled background tint — keeping the type icon, custom title, and foldable `+`/`-` indicator, instead of a literal `[!note]` blockquote. → [usage](docs/usage.md#obsidian-markdown-rendering)
- **Highlighted links** in the rendered markdown view: `[[wikilinks]]`, `[[aliased|links]]`, `![[embeds]]`, and standard `[text](target)` links render in a distinct underlined link colour (an Obsidian-like purple) so they stand out from body text, instead of glow's plain body colour. → [usage](docs/usage.md#obsidian-markdown-rendering)
